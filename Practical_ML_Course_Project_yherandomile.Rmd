...
Out of sample error can be defined as the error on the cases not in the training set. Since the original `testing` set here doesn't contain the true labels, thus it will be impossible to estimate the Out-of-sample error beforehand. To be able to give an estimate for it, I split the initial training set randomly into two -- into training and validation sets and I use the validation set records to estimate the out of sample error. So I could say that the out-of-the-bag error is either 1- accuracy of the model on the validation set or the mean of the cross validation errors (when using `method=cv` of the `caret` package).
